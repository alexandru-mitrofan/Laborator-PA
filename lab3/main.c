...
int comparator(const void* prod1, const void* prod2) {
	double unityValue1 = (double) ((Product *)prod1)->value / ((Product *)prod1)->weight;
	double unityValue2 = (double) ((Product *)prod2)->value / ((Product *)prod2)->weight;
	return sgn(unityValue2 - unityValue1);
}
...
double fractionalKnapsack(Product *arr, int n, int Gmax) {
	...
	double G = 0;
	while(G < Gmax && n > 0) {
		if (Gmax - G < arr[0].weight) {
			finalValue += (Gmax - G) * ((double) arr[0].value / arr[0].weight);
			G = Gmax;
		} else {
			finalValue += arr[0].value;
			G += arr[0].weight;
		}
		arr++;
		n--;
